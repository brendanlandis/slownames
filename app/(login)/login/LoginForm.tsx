    ...
    const handleLogin = async (event) => {
        ...
        axios
            ...
            .catch(function (error) {
                if (!error) {
                    setErrorMessage(`Login didn't work, and the robot won't even say why. Ask Brendan.`);
                    ...
                    setErrorMessage(`Login didn't work. Tell Brendan that the robot says "${error.message}"`);
                } else {
                    setErrorMessage(`Login didn't work. Tell Brendan that the robot says "${error}"`);
                }
            });
    };
    return (
        <form id="loginForm" onSubmit={handleLogin}>
            ...
            <div>
                <InputPass id="loginFormPassword" label="password" labeldisplay={false} />
            </div>
            <div id="errors">{errorMessage}</div>
            <div className="submit-wrapper">
                <ButtonSubmit id="loginFormSubmit" label="login" />
            </div>
        </form>
    );
}
